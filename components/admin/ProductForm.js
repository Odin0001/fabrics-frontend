...
function ImageManager({ slug, images = [], onImagesChange }) {
  ...
  }, [slug, images.length, uploadMutation])
...
const EMPTY = {
  ...
  category: '', is_active: true, is_featured: false, colors: [], catalog: null,
...
export default function ProductForm({ mode, initialData }) {
  ...
  const [form,   setForm]   = useState(() => {
    ...
    return {
      ...EMPTY,
      ...initialData,
      catalog: initialData.catalog?.id ?? initialData.catalog ?? null,
    }
  })
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
        ...
        {/* ── Catalog ───────────────────────────────────────────── */}
        <section className="bg-canvas border border-border p-6">
          <h2 className="text-xs font-medium tracking-[0.15em] uppercase text-gold mb-1">Catalog</h2>
          <p className="text-xs text-muted mb-6">Optional — associate a PDF catalog with this product.</p>
          <div className="relative max-w-sm">
            <select
              value={form.catalog ?? ''}
              onChange={(e) => set('catalog', e.target.value ? Number(e.target.value) : null)}
              className="appearance-none input-base pr-8 text-sm"
            >

...
export default function ProductDetailClient({ initialProduct }) {
  ...
  const downloadImage = async (imgObj, filename) => {
    const url = imageUrl(imgObj)
    if (!url || url === '/placeholder-fabric.jpg') return
    try {
  ...
  const downloadAll = async () => {
    ...
    await Promise.all(
      images.map(async (imgObj, i) => {
        ...
        const blob = await res.blob()
        const ext = blob.type.split('/')[1] || 'jpg'
        zip.file(`${product.sku}-${i + 1}.${ext}`, blob)
      })
    )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
        ...
        <div>
          <div className="relative aspect-square bg-surface-2 mb-4 overflow-hidden group">
            ...
            {img && imageUrl(img) !== '/placeholder-fabric.jpg' && (
              <button
                onClick={() => downloadImage(img, `${product.sku}-${activeImg + 1}`)}
                className="absolute bottom-3 left-3 bg-canvas/90 text-ink text-xs px-2 py-1 flex items-center gap-1.5 hover:bg-canvas transition-colors"
              >
          ...
          {images.length > 1 && (
            <button
              onClick={downloadAll}
              className="mt-3 flex items-center gap-1.5 text-xs text-muted hover:text-ink transition-colors tracking-wide"
            >
              <Download className="h-3.5 w-3.5" />
              Download All
            </button>
        ...
        <div className="animate-fade-up">
          ...
          {/* Catalog section — only shown when a catalog is attached */}
          {product.catalog && (
            <div className="mt-8 border border-border p-5">
              <div className="flex items-center gap-2 mb-4">
                <BookOpen className="h-4 w-4 text-gold" />
                <h2 className="text-xs font-medium tracking-[0.15em] uppercase text-ink">Catalog</h2>
              </div>
              <p className="text-sm text-ink mb-4">{product.catalog.name}</p>
              <div className="flex flex-wrap gap-3">
                <a
                  href={product.catalog.file_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 text-xs border border-gold text-gold px-4 py-2 hover:bg-gold hover:text-white transition-colors"
                >
                ...
                <button
                  onClick={async () => {
                    try {
                      const res = await fetch(product.catalog.file_url)
                      const blob = await res.blob()
                      const blobUrl = URL.createObjectURL(blob)
                      const a = document.createElement('a')
                      a.href = blobUrl
                      a.download = `${product.catalog.name}.pdf`
                      a.click()
                      URL.revokeObjectURL(blobUrl)
                    } catch {
                      window.open(product.catalog.file_url, '_blank')
                    }
                  }}
                  className="flex items-center gap-2 text-xs border border-border text-muted px-4 py-2 hover:border-ink hover:text-ink transition-colors"
                >
                  <Download className="h-3.5 w-3.5" />
                  Download PDF
                </button>
